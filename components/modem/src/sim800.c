...
esp_err_t sim800_handle_response_default(modem_dce_t *dce, const char *line)
{
    ...
    {
        printf("URC: %s", line);
        if (!strncmp(line, "SMS Ready", 6))
            sim800_dce->ready=MODEM_READY;
...
esp_err_t sim800_handle_response_at_command(modem_dce_t *dce, const char *line)
{
   sim800_handle_response_default(dce, line);
   return ESP_OK;
}
...
esp_err_t sim800_at(modem_dce_t *dce, const char *at_command)
{
    ...
    dce->handle_line = sim800_handle_response_at_command;
...
fail:
    ...
    DCE_CHECK(dte->send_cmd(dte, "AT\r", MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", fail);
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "sync failed", err);
    ESP_LOGD(DCE_TAG, "sync ok");
    return ESP_OK;
err:
...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
    ...
    sim800_dce->ready=MODEM_UNKNOWN;
    ...
    static const char *const init_strings[] = {
        "+IPR=0",   /* Enable autobauding if not already enabled. */
        "+IFC=0,0", /* Disable hardware flow control. */
        "+CMEE=2",  /* Enable extended error reporting. */
        "+CLTS=0",  /* Don't sync RTC with network time, it's broken. */
        "+CIURC=1", /* Disable "Call Ready" URC. */
        "&W0",      /* Save configuration. */
        NULL};

    for (const char *const *command = init_strings; *command; command++)
        DCE_CHECK(sim800_at(&(sim800_dce->parent), *command) == ESP_OK, "init command failed", err_io);

    /* Close echo */
    DCE_CHECK(sim800_echo(&(sim800_dce->parent), false) == ESP_OK, "close echo mode failed", err_io);
    /* Get Module name */
    ...
    while (sim800_dce->ready==MODEM_UNKNOWN)
    {
